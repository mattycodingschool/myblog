/* center each dock glyph exactly: square viewBox around its measured bounds.
   the music note is intentionally offset via its hardcoded viewBox, so it's
   excluded here or this would recenter it on load */
document.querySelectorAll('#links a:not([aria-label="music"]) svg').forEach(s => {
  const b = s.getBBox();
...
const TINTS = {
  ...
  music: 'linear-gradient(180deg, #ff8fab, #fb5c74)',
  info: 'tiles',
};
